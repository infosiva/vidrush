...
10. [x] myvitals — 3 fixes: (a) removed 2 dead mixed-content tracker scripts (`http://31.97.56.148:3098/t.js` standard pattern + a 2nd variant, `http://31.97.56.148:3100/script.js` self-hosted-analytics-style tag, both plain `<script>`/`<Script>` tags in layout.tsx). Dead `/api/stats` proxy route present (zero callers) — left out-of-scope. (b) Real bug: onboarding/setup-flow heading in `app/MyVitalsPage.tsx:962` was a `<p>` not `<h1>` — fixed to semantic `<h1>` (confirmed no conflict with the separate real `<h1>` at line 288, which is a mutually-exclusive gated dashboard view). (c) Found+fixed a genuine timing bug in `scripts/e2e-verify.mjs` itself: P1's H1/console-error check ran immediately after `domcontentloaded` with no settle delay, unlike P7/P8 which both wait 800ms — caused false-negative H1 reads on client-rendered headings AND under-reported console errors from async post-load calls. Added matching `waitForTimeout(800)` to P1 (commit c64bc7d, agents root). **New portfolio-wide finding (NOT fixed, scoped as follow-up):** once the E2E timing bug was fixed, P1 surfaced 2 previously-hidden mixed-content console errors — `http://31.97.56.148:3110/guest/usage` and `/admin-code/status` (the guest-access-code auth-api backend, rolled out to 14 projects per the section below). Confirmed via `vercel env pull` that `NEXT_PUBLIC_AUTH_API_URL=http://31.97.56.148:3110` (plain HTTP) is the actual configured production value, not a fallback-masking bug — this is a real infra gap (VPS port 3110 has no TLS termination) affecting all 14 guest-access-code projects, not myvitals-specific. Proper fix needs an HTTPS reverse-proxy (Nginx/Caddy) in front of port 3110 on the VPS, or a per-project Next.js API rewrite/proxy — infra work beyond this wave's scope, needs its own task. Build green, pushed 20e2c37 (trackers) + d778d26 (H1 fix), E2E 9/10 (only failure = the flagged port-3110 issue above, everything else clean, no regression).
11. [x] nammatamil — removed dead mixed-content tracker script (`http://31.97.56.148:3098/t.js`, standard pattern) + unused `Script` import in `src/app/layout.tsx`. No 3100-variant tracker, no port-3110 guest-access-code calls (not on that rollout, confirmed zero console errors), no dead `/api/stats` route, no `var(--foreground)`-on-dark contrast issue. Auto-deploy fired correctly (no manual redeploy needed). Build green, pushed 1bc907ea, E2E 10/10 clean.
12. [x] weekendai — removed dead mixed-content tracker script + unused `Script` import (`app/layout.tsx`). Found+fixed real §M violation: `SharedFooter` links to `/terms` but no `app/terms/page.tsx` existed (404) — built a minimal terms page matching `/privacy`'s style. Dead `/api/stats` route present (not checked for callers this time, consistent with pattern, left out-of-scope). Port-3110 guest-access-code calls present (`lib/shared/useGate.ts`) — same portfolio-wide mixed-content issue flagged on myvitals, not fixed here. Build green, pushed 9d707b5 (tracker) + follow-up (terms page), E2E 9/10 (only failure = port-3110). **Noted, not fixed:** P7 mobile shows `CTA ✗` (primary CTA not visible above fold at 375px) — real §T concern, needs its own look, flagging for a future pass rather than expanding this wave.
13. [x] quizbytesdaily — 2 fixes: (a) removed both dead mixed-content tracker variants in one pass (`http://31.97.56.148:3098/t.js` standard + `http://31.97.56.148:3100/script.js` self-hosted-analytics-style, both in `app/layout.tsx`). Confirmed `/api/stats` is a LIVE route here (unlike every other project checked) — actively called by `app/page.tsx` + `app/admin/page.tsx`, correctly left untouched. (b) Real §L bug: E2E P7 flagged 375px horizontal overflow (391px measured). Root cause: sticky header in `app/page.tsx` combined a `shrink-0` 200px logo + `flex-1` spacer + a `shrink-0` "Daily Quizzes" nav pill with zero responsive sizing — combined width exceeded 375px viewport, pill visibly clipped in screenshot. Fixed by hiding the pill below `sm` breakpoint (`flex` → `hidden sm:flex`) — it's a self-referential link to the homepage itself, safe/low-risk to hide on mobile. Verified fix locally (P7 overflow=none) before push. Build green, pushed 175d47c (trackers) + e282609 (overflow fix). E2E live 9/10 (only failure = port-3110, same portfolio-wide pattern as myvitals/weekendai, not fixed here).
14. [ ] trackwealth
15. [ ] anylocal
